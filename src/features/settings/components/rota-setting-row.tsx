...
type RotaSettingOption = {
  label: string
  value: string
}

function RotaSettingRow({
  children,
  description,
  title,
}: {
...
function RotaSettingSelect({
  ...
  options: Array<RotaSettingOption>
  value: string
}) {
  const selectedLabel =
...
function RotaSettingValue({
  ...
  return (
    <span
      className={cn(
        "text-xs font-bold",
        tone === "default" ? "text-[#46577d]" : "text-emerald-600"
      )}
    >
      {children}
    </span>
