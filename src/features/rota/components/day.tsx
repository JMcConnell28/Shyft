...
function Day({ dayId }: { dayId: string }) {
  const { days, shiftIdsByDayId } = useRotaWorkspace()
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-sm bg-neutral-100 shadow-inner shadow-neutral-200 outline">
      ...
      <div className="min-h-0 flex-1 space-y-1 overflow-x-hidden overflow-y-auto px-1.5 pb-1.5">
        {shiftIds.length === 0 ? (
          ...
          shiftIds.map((shiftId) => <Shift key={shiftId} shiftId={shiftId} />)
        )}
      </div>
    </div>
  )
